...
The prohibition is on **attribution**, not on the substring. This file is named
`CLAUDE.md`, and describing the `cd39c20` mistake requires naming the trailer it
carries — neither claims an AI wrote anything. A grep for these words will match
both; read what it matched before assuming a violation. What is forbidden is a
commit, PR, comment or changelog entry that presents the work as authored,
co-authored, or generated by a model.
